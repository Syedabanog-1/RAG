...
export default function RAGMasterclass() {
  ...
  return (
    <main className="h-screen w-full bg-[#020617] text-white overflow-hidden flex flex-col font-sans selection:bg-sky-500">
      ...
      <nav className="h-20 lg:h-28 px-6 lg:px-16 flex justify-between items-center bg-black/40 backdrop-blur-2xl border-b border-white/10 relative z-50">
        <div className="flex items-center gap-6 lg:gap-8">
          <motion.div 
            whileHover={{ scale: 1.1, rotate: 5 }}
            className="w-12 h-12 lg:w-16 lg:h-16 bg-sky-500 rounded-[1.5rem] flex items-center justify-center shadow-[0_0_30px_rgba(14,165,233,0.5)] cursor-pointer"
          >
             <Cpu className="w-6 h-6 lg:w-8 lg:h-8 text-white" />
          </motion.div>
          <div>
            <h1 className="text-2xl lg:text-4xl font-black uppercase tracking-tighter leading-none">RAG <span className="text-sky-400">MASTERY</span></h1>
            <p className="text-xs lg:text-sm text-slate-400 font-bold tracking-[0.4em] mt-1 uppercase">AI EDUCATION</p>
          </div>
        </div>

        <div className="flex items-center gap-6 lg:gap-10">
          <div className="hidden md:flex items-center gap-4 bg-white/5 border border-white/10 px-6 py-2 rounded-full">
            <span className="text-sm font-black text-slate-500 uppercase tracking-widest">SLIDE</span>
            <span className="text-xl lg:text-2xl font-black text-sky-400">{currentSlide + 1}<span className="text-slate-700 mx-2">/</span>{slides.length}</span>
          </div>

          <div className="flex items-center gap-4">
            <button 
              onClick={() => setShowTopics(true)}
              className="p-4 lg:p-5 glass border border-white/10 rounded-2xl hover:bg-sky-600 transition-all group"
            >
              <BookOpen className="w-6 h-6 lg:w-7 lg:h-7 text-white group-hover:scale-110 transition-transform" />
            </button>
            ...
            <div className="relative">
              <button 
                ...
                className="flex items-center gap-3 px-6 py-4 lg:px-8 lg:py-5 glass border border-white/10 rounded-[2rem] hover:border-sky-500/50 transition-all font-black uppercase text-sm lg:text-base"
              >
                <Globe className="w-5 h-5 lg:w-6 lg:h-6 text-sky-400" />
                {selectedLang.name}
              </button>
              ...
              <AnimatePresence>
                {showSettings && (
                  <motion.div 
                    ...
                    className="absolute top-full right-0 mt-4 w-64 glass border border-white/10 rounded-[2rem] p-3 z-[200] shadow-2xl"
                  >
                    {languages.map((l) => (
                      <button 
                        key={l.code} 
                        onClick={() => { setSelectedLang(l); setShowSettings(false); }}
                        className={`w-full text-left px-6 py-4 rounded-xl text-lg font-black transition-all mb-2 last:mb-0 ${selectedLang.code === l.code ? 'bg-sky-600 text-white' : 'hover:bg-white/10 text-slate-300'}`}
                      >
                        {l.name}
                      </button>
                    ))}
      ...
      {/* Main Content Area */}
      <div className="flex-grow flex items-center px-6 lg:px-20 py-6 lg:py-10 relative overflow-y-auto lg:overflow-hidden">
        <AnimatePresence mode="wait">
          <motion.div 
            ...
            className="w-full grid lg:grid-cols-2 gap-10 lg:gap-20 items-center"
          >
            {/* Left: Text Content */}
            <div className="space-y-8 lg:space-y-12">
              <div className="space-y-4 lg:space-y-6">
                <motion.div 
                  ...
                  className="inline-block px-6 py-2 lg:px-8 lg:py-3 bg-sky-600/20 border-2 border-sky-500/30 rounded-full"
                >
                  <span className="text-sky-400 font-black text-sm lg:text-lg uppercase tracking-[0.2em]">
                    {slide.subtitle || "Mastering RAG Technology"}
                  </span>
                </motion.div>
                
                <h2 className="text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-black leading-[1.1] uppercase tracking-tight">
              ...
              <div className="space-y-4 lg:space-y-6">
                {slide.content.map((item, i) => (
                  ...
                  >
                    <div className="mt-2 w-3 h-3 lg:w-5 lg:h-5 bg-sky-500 rounded-full shadow-[0_0_20px_rgba(14,165,233,1)] flex-shrink-0" />
                    <span className="text-xl lg:text-2xl font-bold text-slate-200 group-hover:text-white leading-tight">{item}</span>
                  </motion.div>
                ))}
              </div>

              {/* Action Buttons */}
              <div className="flex flex-col sm:flex-row gap-6 lg:gap-8 pt-6 lg:pt-8">
                <button 
                  onClick={() => speak(slide)}
                  className={`group flex items-center justify-center w-20 h-20 lg:w-24 lg:h-24 rounded-[2rem] font-black transition-all shadow-2xl ${isSpeaking ? 'bg-rose-600 text-white' : 'bg-gradient-to-br from-sky-500 to-blue-700 text-white hover:scale-105 active:scale-95 shadow-sky-600/40'}`}
                  title="Voice Explanation"
                >
                  {isGenerating ? <Zap className="w-8 h-8 lg:w-10 lg:h-10 animate-spin" /> : isSpeaking ? <Pause className="w-8 h-8 lg:w-10 lg:h-10" /> : <Volume2 className="w-8 h-8 lg:w-10 lg:h-10" />}
                </button>

                <button 
                  onClick={startMovie}
                  className="flex items-center justify-center w-20 h-20 lg:w-24 lg:h-24 rounded-[2rem] border-2 border-white/10 glass hover:bg-white/10 transition-all group"
                  title="Watch Video"
                >
                  <Youtube className="w-10 h-10 lg:w-12 lg:h-12 text-sky-400 group-hover:scale-125 transition-transform" />
                </button>
              </div>
            </div>
            ...
            <div className="relative group hidden lg:block">
              <motion.div 
                className="relative z-10 rounded-[3rem] lg:rounded-[4rem] overflow-hidden border-[8px] border-white/5 shadow-[0_0_80px_rgba(0,0,0,0.8)] aspect-[4/3]"
              >
                <img 
                  src={slide.imageUrl} 
                ...
                <div className="absolute bottom-8 left-8 right-8 lg:bottom-12 lg:left-12 lg:right-12 p-6 lg:p-8 glass border border-white/20 rounded-[2rem]">
                  <p className="text-sm lg:text-base font-black text-sky-400 uppercase tracking-[0.3em] mb-2">Deep Learning Insight</p>
